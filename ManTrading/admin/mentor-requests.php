...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($userId > 0 && in_array($action, ['approve', 'reject'], true)) {
        ...
        $stmt = $conn->prepare('UPDATE users SET mentor_status = ? WHERE id = ?');
        $stmt->bind_param('si', $newStatus, $userId);
        $stmt->execute();
        $stmt->close();

        $stmt = $conn->prepare("UPDATE mentor_requests SET status = ?, responded_at = NOW() WHERE user_id = ? AND status = 'pending'");
        $stmt->bind_param('si', $newStatus, $userId);
        $stmt->execute();
        $stmt->close();

        flash_set($action === 'approve' ? 'User disetujui jadi Mentor.' : 'Pengajuan Mentor ditolak.', 'success');
    }
    header('Location: mentor-requests.php');
    exit;
}

// Daftar user dengan status pending (butuh aksi) + riwayat status lain, plus pesan pengajuan terakhir
$pending = $conn->query("
    SELECT u.id, u.full_name, u.email, u.mentor_status,
           (SELECT r.message FROM mentor_requests r WHERE r.user_id = u.id AND r.status = 'pending' ORDER BY r.id DESC LIMIT 1) AS message
    FROM users u WHERE u.mentor_status = 'pending' ORDER BY u.id DESC
")->fetch_all(MYSQLI_ASSOC);

$others = $conn->query("SELECT id, full_name, email, mentor_status FROM users WHERE mentor_status IN ('approved','rejected') ORDER BY id DESC")->fetch_all(MYSQLI_ASSOC);

$user = current_user();
$activeTab = 'mentor-admin';
$basePath = '../';
$pageTitle = 'ManTrading - Kelola Pengajuan Mentor';
  ...
  <div class="flex-1 flex flex-col min-w-0 lg:overflow-hidden">
    <header class="bg-white px-6 py-5 border-b border-gray-200 flex justify-between items-center z-10 sticky top-0 shadow-sm">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-xl md:text-2xl font-bold text-gray-800 tracking-tight">Kelola Pengajuan Mentor</h1>
          <p class="text-gray-500 text-xs md:text-sm mt-0.5">Setujui atau tolak pengajuan jadi Mentor. Mentor yang disetujui bisa posting di Community.</p>
        </div>
      </div>
    </header>
    ...
    <main class="flex-1 lg:overflow-y-auto p-4 md:p-6 lg:p-8 custom-scrollbar bg-gray-50/50">
      <div class="max-w-4xl mx-auto flex flex-col gap-6 animate-fade-in-up">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          ...
          <?php if (!$pending): ?>
            <p class="p-6 text-sm text-gray-500 text-center">Tidak ada pengajuan Mentor yang menunggu saat ini.</p>
          <?php else: foreach ($pending as $p): ?>
            <div class="p-4 md:p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              <div class="min-w-0">
                <p class="font-bold text-gray-800 text-sm"><?= e($p['full_name']) ?></p>
                <p class="text-xs text-gray-500"><?= e($p['email']) ?></p>
                <?php if (!empty($p['message'])): ?>
                  <p class="text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg p-2 mt-2 whitespace-pre-line break-words max-w-md"><?= e($p['message']) ?></p>
                <?php endif; ?>
              </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          ...
          <?php else: foreach ($others as $o): ?>
            <div class="p-4 md:p-5 border-b border-gray-100 flex items-center justify-between gap-3">
              ...
              <span class="text-[10px] font-bold uppercase px-3 py-1 rounded-full border <?= $o['mentor_status'] === 'approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200' ?>">
                <?= $o['mentor_status'] === 'approved' ? 'Disetujui' : 'Ditolak' ?>
              </span>
            </div>
          <?php endforeach; endif; ?>
        </div>
